...
import type { CvSavePrepareResult } from "@/app/hooks/useCvSessionCapture";
import {
  MiniSquatDetector,
...
function usesStsPauseStop(detector: PatientCvDetector): detector is SitToStandDetector | MiniSquatDetector {
  return typeof (detector as SitToStandDetector).stopPatientTrackingSession === "function";
}
...
export type PatientCvCaptureProps = {
  ...
  onSkipped?: () => void;
  onRegisterMetricsFlush?: (flush: () => void | CvSavePrepareResult) => void;
...
export function PatientCvCapture({
  ...
  const prepareAndFlushForSave = useCallback((): CvSavePrepareResult => {
    const detector = detectorRef.current;
    if (!detector) {
      return { metrics: null };
    }

    let motionSummary: import("@/app/lib/cv/motion-evidence.types").SessionMotionEvidenceSummary | undefined;
    if (exerciseId === "sit-to-stand") {
      const sts = detector as SitToStandDetector;
      sts.ensureMotionEvidenceFinalized();
      const summary = sts.getSessionMotionEvidenceSummary();
      if (summary) motionSummary = summary;
    }

    const metrics = detector.getDerivedMetrics();
    onMetricsUpdateRef.current?.(metrics);

    return { metrics, motionSummary };
  }, [exerciseId]);

  useEffect(() => {
    if (!onRegisterMetricsFlush) return;
    onRegisterMetricsFlush(prepareAndFlushForSave);
  ...
  useEffect(() => {
    ...
    onRegisterMotionSummaryProvider(() => {
      ...
      return (detector as SitToStandDetector).getSessionMotionEvidenceSummary();
    });
  ...
  const handleStopSession = useCallback(() => {
    const detector = detectorRef.current;
    if (stopInProgressRef.current || !detector) return;
    if (!detector.isPreviewActive() && trackingStopped) return;

    stopInProgressRef.current = true;
    reportMetrics();

    if (usesStsPauseStop(detector)) {
      detector.stopPatientTrackingSession();
    } else {
      detector.stop();
      setCameraLive(false);
    }
  ...
  }, [reportMetrics, syncFromDetector, trackingStopped]);
