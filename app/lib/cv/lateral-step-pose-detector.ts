/**
 * Patient portal Lateral Step CV detector — rep counting via MediaPipe Pose.
 * On-device only — no landmarks or video are persisted.
 */

import type { LateralStepDerivedMetrics } from "@/app/lib/cv/bio-0-contracts";
import { DEFAULT_STS_CONFIG, type CvTrackingQuality } from "@/app/lib/cv/bio-0-contracts";
import {
  PATIENT_LATERAL_STEP_POSE_SHELL,
  PATIENT_LATERAL_STEP_READINESS_MS,
...
import { lateralStepStandPhaseFromRepPhase } from "@/app/lib/cv/lateral-step-shift-phase";
import {
...
export type LateralStepPoseDetectorSnapshot = SitToStandDetectorSnapshot;

export {
  formatSitToStandDuration as formatLateralStepDuration,
  mapSitToStandStartError as mapLateralStepStartError,
};
...
export type LateralStepPoseDetectorCallbacks = {
  onSnapshot: (snapshot: LateralStepPoseDetectorSnapshot) => void;
};
...
/**
 * Patient lateral step detector — MediaPipe shell + ankle-rise rep engine.
 */
export class LateralStepPoseDetector {
  private readonly callbacks: LateralStepPoseDetectorCallbacks;
  private readonly shellConfig: typeof PATIENT_LATERAL_STEP_POSE_SHELL;
  private readonly repConfig: LateralStepRepConfig;
  private readonly repEngine: LateralStepDetector;
  ...
  constructor(
    callbacks: LateralStepPoseDetectorCallbacks,
    repConfig: LateralStepRepConfig = PATIENT_LATERAL_STEP_REP_CONFIG,
    shellConfig: typeof PATIENT_LATERAL_STEP_POSE_SHELL = PATIENT_LATERAL_STEP_POSE_SHELL,
  ) {
    this.callbacks = callbacks;
    this.repConfig = repConfig;
    this.shellConfig = shellConfig;
    this.repEngine = new LateralStepDetector(repConfig);
  }

  getSnapshot(): LateralStepPoseDetectorSnapshot {
    const engine = this.repEngine.getSnapshot();
    const standPhase = lateralStepStandPhaseFromRepPhase(engine.repPhase);
    return {
  ...
  async start(video: HTMLVideoElement, canvas: HTMLCanvasElement): Promise<void> {
    ...
    try {
      ...
      this.initPhase = null;
      this.readinessCheckEndMs = performance.now() + PATIENT_LATERAL_STEP_READINESS_MS;
      this.emit();
