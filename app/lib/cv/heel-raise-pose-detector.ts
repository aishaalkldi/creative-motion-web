/**
 * Patient portal Double Heel Raise CV detector — rep counting via MediaPipe Pose.
 * On-device only — no landmarks or video are persisted.
 */

import type { HeelRaiseDerivedMetrics } from "@/app/lib/cv/bio-0-contracts";
...
export type HeelRaisePoseDetectorSnapshot = SitToStandDetectorSnapshot;

export {
  formatSitToStandDuration as formatHeelRaiseDuration,
  mapSitToStandStartError as mapHeelRaiseStartError,
};
...
/**
 * Patient double heel raise detector — MediaPipe shell + ankle-rise rep engine.
 */
export class HeelRaisePoseDetector {
  private readonly callbacks: HeelRaisePoseDetectorCallbacks;
  private readonly shellConfig: typeof PATIENT_HEEL_RAISE_POSE_SHELL;
  private readonly repConfig: HeelRaiseRepConfig;
  private readonly repEngine: HeelRaiseDetector;
  ...
  constructor(
    callbacks: HeelRaisePoseDetectorCallbacks,
    repConfig: HeelRaiseRepConfig = PATIENT_HEEL_RAISE_REP_CONFIG,
    shellConfig: typeof PATIENT_HEEL_RAISE_POSE_SHELL = PATIENT_HEEL_RAISE_POSE_SHELL,
  ) {
    this.callbacks = callbacks;
    this.repConfig = repConfig;
    this.shellConfig = shellConfig;
    this.repEngine = new HeelRaiseDetector(repConfig);
  }
  ...
  private updateReadinessFromLandmarks(landmarks: PoseLandmark[], nowMs: number): void {
    ...
    const quality = evaluateAnkleTrackingQuality(
      landmarks,
