...
describe("buildPtClinicalReportDraft", () => {
    ...
    assert.ok(
      report.sections.some((section) =>
        [...section.paragraphs, ...section.bullets].some((line) =>
          line.toLowerCase().includes("pain when reaching overhead"),
        ),
      ),
    );
  ...
  it("groups approved fields by clinical concept in structured bundle", () => {
    const bundle = buildStructuredClinicalSourceBundle({
      structuredData: ARABIC_STRUCTURED,
      draft: ENGLISH_DRAFT,
    });
    assert.ok(bundle.presentation.some((field) => field.clinicalConcept === "anatomical_region"));
    assert.ok(bundle.functionalLimitations.some((field) => field.clinicalConcept === "functional_daily_impact"));
  });
});
