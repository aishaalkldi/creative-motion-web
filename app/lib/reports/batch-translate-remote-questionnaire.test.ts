...
describe("batchTranslateRemoteQuestionnaire", () => {
  ...
  it("writes _en and _en_ai keys without overwriting originals", async () => {
    const input = {
      assessmentLanguage: "ar",
      pain: {
        chiefComplaint: "ألم في الكتف",
    ...
    const result = await batchTranslateRemoteQuestionnaire(
      input,
      "sk-test",
      async () => ({ ok: true, translation: "The patient reports shoulder pain" }),
    );
    assert.equal(result.structuredData.chiefComplaint_en, "The patient reports shoulder pain");
    assert.equal(result.structuredData.chiefComplaint_en_ai, "The patient reports shoulder pain");
    assert.equal((result.structuredData as typeof input).pain.chiefComplaint, "ألم في الكتف");
    assert.equal(result.structuredData.clinical_translation_status, "review_required");
  });

  it("regenerates _en fields when regenerate is true", async () => {
    const input = {
      assessmentLanguage: "ar",
      chiefComplaint_en: "Old translation",
      pain: {
        chiefComplaint: "ألم في الكتف",
        painLocation: "",
        painScore: "",
    ...
    const result = await batchTranslateRemoteQuestionnaire(
      input,
      "sk-test",
      async () => ({ ok: true, translation: "Updated clinical English" }),
      { regenerate: true },
    );
    assert.equal(result.structuredData.chiefComplaint_en, "Updated clinical English");
    assert.equal((result.structuredData as typeof input).pain.chiefComplaint, "ألم في الكتف");
  });
});
