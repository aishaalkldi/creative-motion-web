-- ============================================================
-- Migration 000 — schema baseline (public.patients, public.assessments)
--
-- This is the extracted, unmodified executable content of the
-- repository-root schema.sql. It was historically applied by hand
-- (Supabase SQL Editor) directly to Production and was never part
-- of the supabase/migrations/ chain, which is why `supabase db push`
-- against a genuinely empty database fails at 002_core_tables.sql
-- (public.patients / public.assessments do not exist yet).
--
-- This file must run first — before 001_providers.sql — so that a
-- fresh, empty Supabase project can initialize from
-- supabase/migrations/ alone, with no manual/undocumented steps.
--
-- No columns, types, constraints, or table names were changed from
-- the original schema.sql. See ./schema.sql (repository root) for
